...
// SmartDSU-IS: Custom message for OP -> ESP32 -> PCM longitudinal control
// Used on UNSUPPORTED_DSU cars (Lexus IS, RC, GS F) with SmartDSU-IS hardware
// check_relay = false because ESP32 consumes this message (no echo expected)
#define TOYOTA_SMART_DSU_IS_TX_MSGS \
  {0x2FE, 0, 8, .check_relay = false},  /* SDSU_ACCEL_CMD: OP -> ESP32 */  \
...
static bool toyota_smart_dsu_is = false;  // SmartDSU-IS hardware detected
...
static bool toyota_tx_hook(const CANPacket_t *msg) {
  const TorqueSteeringLimits TOYOTA_TORQUE_STEERING_LIMITS = {
    ...
    // the EPS faults when the steering angle rate is above a certain threshold for too long.
    // to prevent this,
  ...
  if (msg->bus == 0U) {
    ...
    // SmartDSU-IS: 0x2FE (SDSU_ACCEL_CMD) validation
    // This message is sent from OpenPilot to the ESP32 SmartDSU-IS hardware,
    // which converts it to 0x283 PRE_COLLISION for PCM longitudinal control
    if (msg->addr == 0x2FEU) {
      // Only allow if SmartDSU-IS flag is set
      if (!toyota_smart_dsu_is) {
        tx = false;
      } else {
        // Validate acceleration limits
        // Bytes 0-1 are signed int16 big-endian, scale 0.001 m/s²
        int desired_accel = (msg->data[0] << 8) | msg->data[1];
        desired_accel = to_signed(desired_accel, 16);

        // Check acceleration limits: -3500 to +2000 (in 0.001 m/s² units)
        // This matches TOYOTA_LONG_LIMITS
        bool violation = false;
        if ((desired_accel < -3500) || (desired_accel > 2000)) {
          violation = true;
        }

        // Check controls_allowed (except for cancel requests)
        // Byte 2 bit 2 is CANCEL_REQ
        bool is_cancel = (msg->data[2] & 0x04U) != 0U;

        // Only allow non-zero accel commands if controls_allowed OR if it's a cancel request
        if (!controls_allowed && (desired_accel != 0) && !is_cancel) {
          violation = true;
        }
...
static safety_config toyota_init(uint16_t param) {
  // Stock longitudinal TX messages (no 0x343 actuation, cancel only)
  ...
  // OpenPilot longitudinal TX messages
  ...
  // SmartDSU-IS: Stock longitudinal + 0x2FE for ESP32 communication
  // Used on UNSUPPORTED_DSU cars where we keep STOCK_LONGITUDINAL set
  // but send accel commands via 0x2FE to the SmartDSU-IS hardware
  static const CanMsg TOYOTA_SMART_DSU_IS_TX_MSGS_ARRAY[] = {
    TOYOTA_COMMON_TX_MSGS
    TOYOTA_SMART_DSU_IS_TX_MSGS
  };
  ...
  // Select TX message set based on configuration
  if (toyota_smart_dsu_is) {
    // SmartDSU-IS: Use stock TX msgs + 0x2FE for ESP32 communication
    // Note: STOCK_LONGITUDINAL should be set to avoid relay check on 0x343
    SET_TX_MSGS(TOYOTA_SMART_DSU_IS_TX_MSGS_ARRAY, ret);
