...
def create_sdsu_accel_command(accel: float, permit_braking: bool, release_standstill: bool,
  ...
  Sent from OpenPilot to SmartDSU-IS ESP32 which converts to 0x283 for PCM.
  """
  ...
  accel_raw = int(accel_clamped * 1000)
  if accel_raw < 0:
    accel_raw = accel_raw & 0xFFFF
  dat[0] = (accel_raw >> 8) & 0xFF
  dat[1] = accel_raw & 0xFF
  ...
  # Byte 7: CHECKSUM
  checksum = sum(dat[0:7]) + 0x02 + 0xFE + 8
  dat[7] = checksum & 0xFF
