...
class CarController(CarControllerBase, SecOCLongCarController):
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    # SmartDSU-IS: Counter for 0x2FE messages
    self.sdsu_counter = 0
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % 3 == 0:
        ...
        # SmartDSU-IS: Send 0x2FE to ESP32 interceptor
        if self.CP.autoResumeSng and self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
          can_sends.append(toyotacan.create_sdsu_accel_command(
            pcm_accel_cmd, self.permit_braking, not self.standstill_req,
            pcm_cancel_cmd, CC.longActive, self.sdsu_counter))
